...
TEST(api, test_delegate_pub_key)
{
	...
	auto delegate = arkManager.delegate(darkPubkey);
...
TEST(api, test_delegate_voters)
{
	...
	if (voters.count() > 0)
	{
		...
		ASSERT_STREQ("DHQ4Fjsyiop3qBR4otAjAu6cBHkgRELqGA", voters[1].address().getValue());
		ASSERT_STREQ("0275776018638e5c40f1b922901e96cac2caa734585ef302b4a2801ee9a338a456", voters[1].public_key());
	};
}
