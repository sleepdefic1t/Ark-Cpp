...
{
	const auto account = ARK::Crypto::createAccount(ARK::Constants::Networks::Network_ADV::devnet.pubKeyHash, passphrase);
...
TEST(crypto, generate_address)
{
	...
	ASSERT_STREQ(
		...
		ARK::Crypto::getAddress(ARK::Constants::Networks::Network_ADV::devnet.pubKeyHash, ParseHex("0361eba308995231b03f005323fa71fc8dcda34564800fde41d166222d6811ebf6")).c_str()
	);

	// main net
	ASSERT_STREQ(
		"AQf97MEVnnJSYsJHfcrLTGH5J4GYSB5dLH",
		ARK::Crypto::getAddress(ARK::Constants::Networks::Network_ADV::main.pubKeyHash, ParseHex("02579b22787db8a7cb838729ad21bb0471e472027904df3d674ef074006a9a22c0")).c_str()
	);

	// test net ??
	//ASSERT_STREQ(
	//	"",
	//	ARK::Crypto::getAddress(ARK::Constants::Networks::Network_ADV::testnet.pubKeyHash, ParseHex("")).c_str()
	//);
}
...
TEST(crypto, get_private_key)
{
	...
	ARK::Crypto::getPrivkey(phrase, priv_key);
...
TEST(crypto, get_public_key)
{
	...
	ARK::Crypto::getPubkey(priv_key, pub_key, true);
	...
	ARK::Crypto::getPubkey(priv_key, pub_key, false);
...
TEST(crypto, toWIF)
{
	...
	auto wif = ARK::Crypto::toWIF(ARK::Constants::Networks::Network_ADV::main.wif, priv_key, true);
	ASSERT_STREQ(
	...
	std::memcpy(priv_key, &priv_key_vect[0], sizeof(priv_key));
	wif = ARK::Crypto::toWIF(ARK::Constants::Networks::Network_ADV::main.wif, priv_key, true);
	ASSERT_STREQ(
	...
	wif = ARK::Crypto::toWIF(ARK::Constants::Networks::Network_ADV::bitcoin.wif, priv_key, true);
	ASSERT_STREQ(
	...
	ARK::Crypto::getPrivkey(passphrase, priv_key_vect);
	std::memcpy(priv_key, &priv_key_vect[0], sizeof(priv_key));
	wif = ARK::Crypto::toWIF(ARK::Constants::Networks::Network_ADV::devnet.wif, priv_key, true);
	ASSERT_STREQ(
...
TEST(crypto, fromWIF)
{
	...
	uint8_t version;
	bool compressed;	
	ARK::Crypto::fromWIF("S9aCCSFvm8kNeyFb1t6pLb5oJs9tv96ag6uA8Du6UM7zsmsNHQiz", version, priv_key, compressed);
	ASSERT_STREQ(
	...
	ARK::Crypto::fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", version, priv_key, compressed);
	ASSERT_STRCASEEQ(
	...
	ARK::Crypto::fromWIF("SDCe8styqokHi4pSe5jVRiYVV63Mef2TGsE1D4HhtGAL1DytHLtd", version, priv_key, compressed);
	ASSERT_STRCASEEQ(
	...
	ARK::Crypto::getPrivkey(passphrase, expected_priv_key);
	ARK::Crypto::fromWIF("SEZuJZouNK8GLXNApjciH4QnSKiNr971exVcL2Y6XfrDF5o977zB", version, priv_key, compressed);
